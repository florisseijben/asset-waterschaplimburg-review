---
title: Vrij meanderende watergang
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/vrij-meanderend
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-08
summary: Op deze pagina vind je de definitie en afbakening van Vrij meanderende watergang, de samenhang met Watergangsectie en de geometrische uitgangspunten.
heroTitle: Vrij meanderende watergang
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Vrij meanderende watergang
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - ecologie-en-groen
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-ecologie-en-groen
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een vrij meanderende watergang is een type watergangsectie waarin de ligging en profielvorm natuurlijker kunnen variëren en niet strak door een vast profiel worden bepaald.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Vrij meanderende watergang
    text: Type naar compositie waarin natuurlijke variatie in ligging en profiel kenmerkend is.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Vrij meanderende watergang een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Meandering
    text: Natuurlijk slingerend verloop van een watergang.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Vrij meanderende watergang een sectie waarin dynamiek, profielvariatie en ecologische ontwikkeling een grotere rol spelen.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Vrij meanderende watergang
  - label: Bovenliggend object
    value: Watergangsectie
contentSections:
  - title: Overzicht / Samenhang
    summary: Vrij meanderende watergang is een type watergangsectie waarin de watergang meer ruimte krijgt om natuurlijk te verlopen.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject en houdt rekening met de ruimtelijke zone waarbinnen de watergang kan slingeren of profielvariatie kan optreden.
  - title: Onderdelen
    summary: De belangrijkste onderdelen zijn de meanderzone, bodem, taluds of oevers, bermen en de ruimtelijke begrenzing van de sectie.
    items:
      - title: Meanderzone
        text: Zone waarbinnen het verloop en de profielvorm van de watergang variatie kunnen vertonen.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/meanderzone
      - title: Oever
        text: Randzone waar waterprofiel en omgeving op elkaar aansluiten.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/oever
      - title: Berm
        text: Beheerbare strook of overgangszone langs de meanderzone of oever.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/berm
  - title: Geometrie
    summary: De geometrie wordt bepaald door het deeltraject, de meanderzone en de profielvariatie binnen de sectie.
    items:
      - title: Dwarsprofiel
        text: Het dwarsprofiel laat de globale profielopbouw van een vrij meanderende watergang zien.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_vrij meanderende watergang/Vrij meanderende watergang.png
          alt: Doorsnede van Vrij meanderende watergang.
        caption: Afbeelding 1. Dwarsprofiel van Vrij meanderende watergang.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties en meanderende watergangen.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Gestandaardiseerde eigenschappen en relaties van dit sectietype.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Voorbeelddata voor vrij meanderende watergangen.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van natuurlijke profielvariatie.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere sectietypen.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Vrij meanderende watergang` uit als type naar compositie van `Watergangsectie`.
