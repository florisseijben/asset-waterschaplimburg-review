---
title: Bronloop
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/bronloop
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-08
summary: Op deze pagina vind je de definitie en afbakening van Bronloop, de samenhang met Watergangsectie en de geometrische uitgangspunten.
heroTitle: Bronloop
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Bronloop
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - ecologie-en-groen
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-ecologie-en-groen
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een bronloop is een type watergangsectie aan het begin of in de bovenloop van een watergang, waar de watergang gevoed wordt door bron- of bovenstroomse afvoer.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Bronloop
    text: Type naar compositie voor een bron- of bovenloopsectie van een watergang.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Bronloop een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Bovenloop
    text: Begintraject of hoger gelegen trajectdeel van een watergang.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Bronloop een watergangsectie waarin de start, toevoer en profielopbouw van de watergang centraal staan.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Bronloop
  - label: Bovenliggend object
    value: Watergangsectie
contentSections:
  - title: Overzicht / Samenhang
    summary: Bronloop is een type watergangsectie dat de bron- of bovenloop van een watergang beschrijft.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject waar de bronloop begint en waar de sectie overgaat in een opvolgende watergangsectie.
  - title: Onderdelen
    summary: De belangrijkste onderdelen zijn het bron- of bovenloopprofiel, de bodem, taluds of oevers en de profielgrenzen.
    items:
      - title: Bovenloopprofiel
        text: Profieldeel dat kenmerkend is voor het begintraject van de watergang.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/bovenloopprofiel
      - title: Profielgrens
        text: Begrenzing waarmee de overgang naar de omgeving en opvolgende secties wordt vastgelegd.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/profielgrens
  - title: Geometrie
    summary: De geometrie wordt bepaald door het begintraject, de profielvorm en de aansluiting op bovenstroomse toevoer of opvolgende watergangsecties.
    items:
      - title: Dwarsprofiel
        text: Het dwarsprofiel laat de profielopbouw van een bronloopsectie zien.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_bronloop/Bronloop profiellijn.png
          alt: Doorsnede van Bronloop met profiellijn.
        caption: Afbeelding 1. Dwarsprofiel van Bronloop.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor bronlopen en watergangsecties.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Gestandaardiseerde eigenschappen en relaties van dit sectietype.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Voorbeelddata voor bronloopsecties.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van bronlopen.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere sectietypen.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Bronloop` uit als type naar compositie van `Watergangsectie`.
