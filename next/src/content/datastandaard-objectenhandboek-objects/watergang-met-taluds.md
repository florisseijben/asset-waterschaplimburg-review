---
title: Watergang met taluds
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/met-taluds
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-05
summary: Op deze pagina vind je de definitie en afbakening van Watergang met taluds, de samenhang met Watergangsectie en Talud, de geometrische uitgangspunten en de productrelaties.
heroTitle: Watergang met taluds
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Watergang met taluds
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - ecologie-en-groen
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-ecologie-en-groen
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een watergang met taluds is een type watergangsectie waarbij de zijdelingse begrenzing van het profiel wordt gevormd door taluds.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Watergang met taluds
    text: Type naar compositie van Watergangsectie waarin taluds de primaire profielbegrenzing vormen.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Watergang met taluds een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Talud
    text: Profielonderdeel dat de schuine overgang tussen bodem en maaiveld of rand vormt.
    href: /datastandaard/woordenboek
  - title: Dwarsprofiel
    text: Begrip waarmee de opbouw van bodem, taluds en aansluitende randen binnen de sectie leesbaar wordt.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Watergang met taluds een watergangsectie waarvan de profielopbouw primair wordt bepaald door bodem en taluds. Het type vormt de ingang naar verdere uitwerking van talud, bodem, bekleding en beheerkenmerken.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Watergang met taluds
  - label: Bovenliggend object
    value: Watergangsectie
  - label: Bronsysteem
    value: semantische beheeromgeving
contentSections:
  - title: Overzicht / Samenhang
    summary: Watergang met taluds is een type watergangsectie waarin taluds aan een of beide zijden de overgang vormen tussen de bodem van de watergang en de aansluitende omgeving.
    image:
      src: /images/objectenhandboek/watersysteem/watergangen/watergang sectie.jpg
      alt: Schematische weergave van een watergangsectie met taluds binnen een watergang.
    caption: Afbeelding 1. Watergang met taluds binnen de watergangsectie.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject van de watergangsectie en omvat de bodem, de taluds en de aansluitende profielgrenzen die nodig zijn om het dwarsprofiel eenduidig te beschrijven.
  - title: Onderdelen
    summary: De belangrijkste onderdelen binnen dit type zijn de taluds, de bodem en eventuele constructieve of beheergerelateerde uitwerkingen op het profiel.
    items:
      - title: Talud
        text: Schuin profielonderdeel dat de overgang vormt tussen bodem en maaiveld of rand.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/talud
      - title: Bodem
        text: Onderste profieldeel van de watergangsectie dat samen met de taluds het dwarsprofiel vormt.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/bodem
  - title: Geometrie
    summary: De geometrie van Watergang met taluds wordt bepaald door het deeltraject, de profielgrenzen, de bodemligging en de taludhellingen binnen het dwarsprofiel.
    items:
      - title: Overzicht
        text: Het overzicht toont de watergangsectie met taluds als samenhang tussen profiel, begrenzing en ligging binnen de watergang.
        image:
          src: /images/objectenhandboek/watersysteem/watergangen/geometrie.png
          alt: Geometrische weergave van een watergangsectie met taluds, profielgrenzen en dwarsprofiel.
        caption: Afbeelding 2. Geometrie van Watergang met taluds.
        blocks:
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds overzicht.jpg
              alt: Overzicht van Watergang met taluds met profielopbouw en begrenzing.
            caption: Afbeelding 3. Overzicht van Watergang met taluds.
      - title: Dwarsprofiel
        text: Het dwarsprofiel maakt zichtbaar hoe profiellijnen en profielvlakken de bodem, taluds en aansluitende grenzen beschrijven.
        blocks:
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds profiellijnen.png
              alt: Dwarsprofiel van Watergang met taluds met profiellijnen.
            caption: Afbeelding 4. Dwarsprofiel met profiellijnen.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds profielvlakken.png
              alt: Dwarsprofiel van Watergang met taluds met profielvlakken.
            caption: Afbeelding 5. Dwarsprofiel met profielvlakken.
      - title: Bovenaanzicht
        text: Het bovenaanzicht toont de ligging van de watergangsectie met taluds in de lengterichting van de watergang.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds bovenaanzicht.png
          alt: Bovenaanzicht van Watergang met taluds.
        caption: Afbeelding 6. Bovenaanzicht van Watergang met taluds.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties, taluds en profielopbouw.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Dit product definieert de eigenschappen, relaties en het gedrag van een bepaald type object in een gestandaardiseerde vorm.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Referentieprofielen en voorbeelddata voor watergangsecties met taluds.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van profielopbouw, taludhelling en geometrie.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Talud
    text: Door naar het profielonderdeel dat de schuine begrenzing van dit type vormt.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/talud
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere typen naar compositie.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Watergang met taluds` uit als type naar compositie van `Watergangsectie`.

Doel:

- het type watergangsectie met taluds als eigen objectpagina bereikbaar maken;
- de relatie met talud, bodem en profielgrenzen expliciet maken;
- en dezelfde objectpagina-indeling hanteren als de bovenliggende Watergangsectie.
