---
title: Watergang met wandconstructie
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/met-wandconstructie
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-08
summary: Op deze pagina vind je de definitie en afbakening van Watergang met wandconstructie, de samenhang met Watergangsectie en de geometrische uitgangspunten.
heroTitle: Watergang met wandconstructie
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Watergang met wandconstructie
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een watergang met wandconstructie is een type watergangsectie waarbij een verticale of semi-verticale wandconstructie de begrenzing van het profiel vormt.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Watergang met wandconstructie
    text: Type naar compositie waarin een wandconstructie de profielbegrenzing of constructieve opbouw mede bepaalt.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Watergang met wandconstructie een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Wandconstructie
    text: Constructieve verticale of semi-verticale begrenzing langs het waterprofiel.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Watergang met wandconstructie een compacte of constructief begrensde watergangsectie.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Watergang met wandconstructie
  - label: Bovenliggend object
    value: Watergangsectie
contentSections:
  - title: Overzicht / Samenhang
    summary: Watergang met wandconstructie is een type watergangsectie waarin een wandconstructie de overgang tussen waterprofiel en omgeving bepaalt.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject van de watergangsectie en omvat de wandconstructie voor zover deze de profielgrens of constructieve opbouw vormt.
  - title: Onderdelen
    summary: De belangrijkste onderdelen zijn het waterprofiel, de wandconstructie en de aansluiting op bodem of randzone.
    items:
      - title: Wandconstructie
        text: Constructieve begrenzing die het profiel compact of steiler maakt.
        href: /datastandaard/woordenboek
      - title: Bodem
        text: Onderste profieldeel waarop de wandconstructie kan aansluiten.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/bodem
  - title: Geometrie
    summary: De geometrie wordt bepaald door de ligging van de wandconstructie, de bodemligging en de ruimtelijke begrenzing van de sectie.
    items:
      - title: Dwarsprofiel
        text: Het dwarsprofiel laat zien hoe de wandconstructie het waterprofiel begrenst.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met wandconstructie/Watergang met wandconstructie profiellijn.png
          alt: Doorsnede van Watergang met wandconstructie met profiellijn.
        caption: Doorsnede. Watergang met wandconstructie.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties en wandconstructies.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Gestandaardiseerde eigenschappen en relaties van dit sectietype.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Voorbeelddata voor watergangsecties met wandconstructie.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van profiel en wandconstructie.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere sectietypen.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Watergang met wandconstructie` uit als type naar compositie van `Watergangsectie`.
