---
title: Aandrijving en bewegingswerk
slug: /datastandaard/objectenhandboek/watersysteem/kunstwerken/stuw/aandrijving-en-bewegingswerk
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-20
summary: Op deze pagina vind je de definitie en afbakening van Aandrijving en bewegingswerk en de typen die binnen de stuwdecompositie worden onderscheiden.
heroTitle: Aandrijving en bewegingswerk
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Kunstwerk
  - Stuw
  - Aandrijving en bewegingswerk
classification:
  systems:
    - watersysteem
  disciplines:
    - werktuigbouwkunde
    - procesautomatisering
    - informatie-en-data
  objectFamilies:
    - kunstwerken
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-werktuigbouwkunde
    - objectenhandboek-procesautomatisering
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Aandrijving en bewegingswerk omvat de voorzieningen waarmee het regelmiddel van een stuw wordt bewogen en bediend.
definitionSource: "Werkdefinitie voor de stuwdecompositie binnen het Objectenhandboek."
terms:
  - title: Stuw
    text: Bovenliggend kunstwerktype waarin Aandrijving en bewegingswerk als onderdeel wordt onderscheiden.
    href: /datastandaard/objectenhandboek/watersysteem/kunstwerken/stuw
contentSections:
  - title: Overzicht / Samenhang
    summary: Binnen de decompositie van Stuw vormt Aandrijving en bewegingswerk de technische schakel tussen regeling, bediening en beweging van het regelmiddel.
  - title: Afbakening
    summary: De afbakening volgt de mechanische en elektrische voorzieningen die direct bijdragen aan het verplaatsen van klep, schuif, schotbalken of balg.
  - title: Onderdelen
    summary: Binnen Aandrijving en bewegingswerk worden onderstaande uitvoeringsvormen onderscheiden.
    items:
      - title: Handmatige aandrijving
        text: Bediening met lokale handkracht of handwerktuigen.
        image:
          src: /images/objectenhandboek/Stuw/stuw handmatig regelbaar.jpg
          alt: Handmatig regelbare stuw.
        caption: Handmatige aandrijving.
      - title: Mechanische automatische aandrijving
        text: Beweging met mechanische voorzieningen die automatisch kunnen regelen.
        image:
          src: /images/objectenhandboek/Stuw/stuw mechanisch automatisch regelbaar.jpg
          alt: Mechanisch automatisch regelbare stuw.
        caption: Mechanische automatische aandrijving.
      - title: Elektrische automatische aandrijving
        text: Beweging met elektrische aandrijving die automatisch kan regelen.
        image:
          src: /images/objectenhandboek/Stuw/stuw electrisch-automatisch regelbaar.jpg
          alt: Elektrisch automatisch regelbare stuw.
        caption: Elektrische automatische aandrijving.
productRelations:
  - title: Object Type Library
    text: Modellering van eigenschappen en relaties van Aandrijving en bewegingswerk.
    href: /datastandaard/otl
nextSteps:
  - title: Stuw
    text: Terug naar het bovenliggende kunstwerktype.
    href: /datastandaard/objectenhandboek/watersysteem/kunstwerken/stuw
    direction: back
---

Deze detailpagina werkt `Aandrijving en bewegingswerk` uit als onderdeel van `Stuw`.
