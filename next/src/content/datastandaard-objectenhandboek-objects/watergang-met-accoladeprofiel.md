---
title: Watergang met accoladeprofiel
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/met-accoladeprofiel
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-08
summary: Op deze pagina vind je de definitie en afbakening van Watergang met accoladeprofiel, de samenhang met Watergangsectie en de geometrische uitgangspunten.
heroTitle: Watergang met accoladeprofiel
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Watergang met accoladeprofiel
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - ecologie-en-groen
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-ecologie-en-groen
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een watergang met accoladeprofiel is een type watergangsectie met een profielvorm waarin verschillende profielzones samen de waterafvoer, onderhoudbaarheid en ruimtelijke overgang bepalen.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Watergang met accoladeprofiel
    text: Type naar compositie waarin de profielvorm als accoladeprofiel bepalend is voor de sectie.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Watergang met accoladeprofiel een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Accoladeprofiel
    text: Profielvorm met zones voor waterafvoer, overgang en onderhoud.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Watergang met accoladeprofiel een watergangsectie waarbij de doorsnede meer profielvariatie kent dan een eenvoudig taludprofiel.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Watergang met accoladeprofiel
  - label: Bovenliggend object
    value: Watergangsectie
contentSections:
  - title: Overzicht / Samenhang
    summary: Watergang met accoladeprofiel is een type watergangsectie waarin de profielvorm meerdere zones combineert binnen een samenhangende doorsnede.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject en de profielgrenzen die nodig zijn om de zones van het accoladeprofiel eenduidig te beschrijven.
  - title: Onderdelen
    summary: De belangrijkste onderdelen zijn bodem, taludzones, bermen, overgangszones en profielgrenzen.
    items:
      - title: Profielzone
        text: Deel van het dwarsprofiel met een eigen functie in afvoer, onderhoud of overgang.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/profielzone
      - title: Talud
        text: Schuin profielonderdeel dat de overgang binnen het profiel mede bepaalt.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/talud
      - title: Berm
        text: Beheerbare strook of overgangszone binnen of langs het accoladeprofiel.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/berm
  - title: Geometrie
    summary: De geometrie wordt bepaald door de ligging van de profielzones, bodem, taluds en profielgrenzen binnen de doorsnede.
    items:
      - title: Dwarsprofiel
        text: Het dwarsprofiel laat de kenmerkende profielvorm van het accoladeprofiel zien.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met accoladeprofiel/Watergang met accoladeprofiel profiellijn.png
          alt: Doorsnede van Watergang met accoladeprofiel met profiellijn.
        caption: Afbeelding 1. Dwarsprofiel van Watergang met accoladeprofiel.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties en profielvormen.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Gestandaardiseerde eigenschappen en relaties van dit sectietype.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Voorbeelddata voor watergangsecties met accoladeprofiel.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van profielvormen.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere sectietypen.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Watergang met accoladeprofiel` uit als type naar compositie van `Watergangsectie`.
