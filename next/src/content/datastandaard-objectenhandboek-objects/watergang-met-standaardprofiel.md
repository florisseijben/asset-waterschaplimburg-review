---
title: Watergang met standaardprofiel
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/met-standaardprofiel
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-13
summary: Op deze pagina vind je de definitie en afbakening van Watergang met standaardprofiel, de samenhang met Watergangsectie en de ruimtelijke beschrijving van profielopbouw, onderhoudspad en beschoeiing.
heroTitle: Watergang met standaardprofiel
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Watergang met standaardprofiel
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - ecologie-en-groen
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-ecologie-en-groen
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een watergang met standaardprofiel is een type watergangsectie waarin het reguliere profiel van bodem, taluds, profielgrenzen en eventuele standaardvoorzieningen zoals onderhoudspad of beschoeiing in samenhang wordt beschreven.
definitionSource: "Werkdefinitie voor de samengevoegde uitwerking van standaardprofielen binnen Watergangsectie."
terms:
  - title: Watergang met standaardprofiel
    text: Type naar compositie van Watergangsectie waarin de reguliere profielopbouw centraal staat.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Watergang met standaardprofiel een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Talud
    text: Profielonderdeel dat de schuine overgang tussen bodem en maaiveld of rand vormt.
    href: /datastandaard/woordenboek
  - title: Onderhoudspad
    text: Bereikbare strook of pad dat beheer en onderhoud langs de watergang mogelijk maakt.
    href: /datastandaard/woordenboek
  - title: Beschoeiing
    text: Constructieve oeverbescherming of begrenzing langs de watergang.
    href: /datastandaard/woordenboek
  - title: Dwarsprofiel
    text: Begrip waarmee de opbouw van bodem, taluds en aansluitende randen binnen de sectie leesbaar wordt.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn bundelt Watergang met standaardprofiel de eerdere uitwerkingen voor taluds, onderhoudspad en beschoeiing. Deze elementen worden niet als afzonderlijke objecttypepagina's behandeld, maar als profielonderdelen en standaardvoorzieningen binnen dezelfde watergangsectie.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Watergang met standaardprofiel
  - label: Bovenliggend object
    value: Watergangsectie
  - label: Bronsysteem
    value: semantische beheeromgeving
contentSections:
  - title: Overzicht / Samenhang
    summary: Watergang met standaardprofiel beschrijft de reguliere ruimtelijke opbouw van een watergangsectie. Taluds, bodem, onderhoudspad en eventuele beschoeiing worden binnen een en dezelfde profieluitwerking gelezen.
    image:
      src: /images/objectenhandboek/watersysteem/watergangen/watergang sectie.jpg
      alt: Schematische weergave van een watergangsectie binnen een watergang.
    caption: Afbeelding 1. Watergangsectie binnen de watergang.
    blocks:
      - image:
          src: /images/objectenhandboek/watersysteem/watergangen/geometrie.png
          alt: Geometrische weergave van een watergangsectie met profielgrenzen en dwarsprofiel.
        caption: Afbeelding 5. Geometrie van de watergangsectie.
  - title: Afbakening
    summary: De afbakening volgt het deeltraject van de watergangsectie en omvat de bodem, taluds, profielgrenzen, het onderhoudspad en beschoeiing voor zover die nodig zijn om het standaardprofiel eenduidig te beschrijven.
  - title: Onderdelen
    summary: De belangrijkste onderdelen binnen dit type zijn bodem, taluds, profielgrenzen en standaardvoorzieningen zoals onderhoudspad en beschoeiing.
    items:
      - title: Talud
        text: Schuin profielonderdeel dat de overgang vormt tussen bodem en maaiveld of rand.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/talud
      - title: Bodem
        text: Onderste profieldeel van de watergangsectie dat samen met de taluds het dwarsprofiel vormt.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/bodem
      - title: Onderhoudspad
        text: Bereikbare strook of pad dat beheer en onderhoud langs de watergang mogelijk maakt.
        href: /datastandaard/woordenboek
      - title: Beschoeiing
        text: Constructief onderdeel dat de oever of profielrand ondersteunt of beschermt.
        href: /datastandaard/woordenboek
      - title: Profielgrens
        text: Begrenzing waarmee de ruimtelijke afbakening van de sectie leesbaar wordt.
        href: /datastandaard/woordenboek
  - title: Geometrie
    summary: De geometrie van Watergang met standaardprofiel wordt bepaald door het deeltraject, de profielgrenzen, de bodemligging, de taludhellingen en de positie van onderhoudspad of beschoeiing binnen het dwarsprofiel.
    items:
      - title: Overzicht
        text: De overzichtsbeelden tonen hoe profiel, begrenzing, onderhoud en constructieve oeverbescherming binnen dezelfde watergangsectie samenhangen.
        blocks:
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds overzicht.jpg
              alt: Overzicht van een standaardprofiel met taluds, profielopbouw en begrenzing.
            caption: Afbeelding 2. Standaardprofiel met taluds.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met onderhoudspad/Watergang met onderhoudspad overzicht.jpg
              alt: Overzicht van een standaardprofiel met waterprofiel en onderhoudspad.
            caption: Afbeelding 3. Standaardprofiel met onderhoudspad.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met beschoeiing/watergang met beschoeiing overzicht-optimized.jpg
              alt: Overzicht van een standaardprofiel met constructieve profielbegrenzing.
            caption: Afbeelding 4. Standaardprofiel met beschoeiing.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met onderhoudspad/watergang met onderhoudspad overzicht_2.jpg
              alt: Aanvullend overzicht van een standaardprofiel met onderhoudspad.
            caption: Afbeelding 6. Aanvullend overzicht met onderhoudspad.
      - title: Dwarsprofiel
        text: Het dwarsprofiel maakt zichtbaar hoe profiellijnen en profielvlakken de bodem, taluds, onderhoudszone en aansluitende grenzen beschrijven.
        blocks:
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds profiellijnen.png
              alt: Dwarsprofiel van een standaardprofiel met profiellijnen.
            caption: Afbeelding 7. Dwarsprofiel met profiellijnen.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds profielvlakken.png
              alt: Dwarsprofiel van een standaardprofiel met profielvlakken.
            caption: Afbeelding 8. Dwarsprofiel met profielvlakken.
          - image:
              src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met onderhoudspad/Watergang met onderhoudspad profiellijn.png
              alt: Doorsnede van een standaardprofiel met onderhoudspad.
            caption: Afbeelding 9. Dwarsprofiel met onderhoudspad.
      - title: Bovenaanzicht
        text: Het bovenaanzicht toont de ligging van de watergangsectie in de lengterichting van de watergang.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met taluds/watergang met taluds bovenaanzicht.png
          alt: Bovenaanzicht van een standaardprofiel binnen een watergangsectie.
        caption: Afbeelding 10. Bovenaanzicht van het standaardprofiel.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties, profielopbouw, taluds, onderhoudspaden en beschoeiing.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Dit product definieert de eigenschappen, relaties en het gedrag van een bepaald type object in een gestandaardiseerde vorm.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Referentieprofielen en voorbeelddata voor watergangsecties met standaardprofiel.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van profielopbouw, taludhelling, onderhoudspad, beschoeiing en geometrie.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Talud
    text: Door naar het profielonderdeel dat de schuine begrenzing van het standaardprofiel vormt.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/talud
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere typen naar compositie.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Watergang met standaardprofiel` uit als type naar compositie van `Watergangsectie`.

Doel:

- de eerdere profielvarianten met taluds, onderhoudspad en beschoeiing op een pagina samenbrengen;
- de relatie met talud, bodem, onderhoudspad, beschoeiing en profielgrenzen expliciet maken;
- en dezelfde objectpagina-indeling hanteren als de bovenliggende Watergangsectie.
