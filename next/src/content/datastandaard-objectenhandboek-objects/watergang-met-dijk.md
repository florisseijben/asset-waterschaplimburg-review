---
title: Watergang met dijk
slug: /datastandaard/objectenhandboek/watersysteem/watergangsectie/met-dijk
part: datastandaard
product: objectenhandboek
ownerTeam: datastandaard
status: eerste-template
lastReviewed: 2026-05-08
summary: Op deze pagina vind je de definitie en afbakening van Watergang met dijk, de samenhang met Watergangsectie en de geometrische uitgangspunten.
heroTitle: Watergang met dijk
hierarchy:
  - Watersysteem
  - Stroomgebied
  - Watergang
  - Watergangsectie
  - Watergang met dijk
classification:
  systems:
    - watersysteem
  disciplines:
    - civiele-techniek
    - terreinbeheer-openbare-ruimte
    - informatie-en-data
  objectFamilies:
    - watergangen
    - profielonderdelen
  publications:
    - objectenhandboek-watersysteem
    - objectenhandboek-civiele-techniek
    - objectenhandboek-terreinbeheer-openbare-ruimte
    - objectenhandboek-informatie-en-data
  confidence: afgeleid
definition: Een watergang met dijk is een type watergangsectie waarbij een dijk of waterkerend profiel onderdeel is van de ruimtelijke opbouw langs de watergang.
definitionSource: "Werkdefinitie voor de eerste uitwerking van typen naar compositie binnen Watergangsectie."
terms:
  - title: Watergang met dijk
    text: Type naar compositie waarin een dijklichaam of waterkerend profiel samenhangt met de watergangsectie.
    href: /datastandaard/woordenboek
  - title: Watergangsectie
    text: Bovenliggend objecttype waarvan Watergang met dijk een specifieke uitwerking is.
    href: /datastandaard/woordenboek
  - title: Dijk
    text: Waterkerend grondlichaam of profiel dat onderdeel kan zijn van de ruimtelijke begrenzing.
    href: /datastandaard/woordenboek
contextNote: Binnen deze objectlijn beschrijft Watergang met dijk een watergangsectie waar waterafvoer en waterkerende ruimtelijke opbouw direct met elkaar samenhangen.
metadata:
  - label: Systeem
    value: Watersysteem
  - label: Subsysteem
    value: Stroomgebied
  - label: Objecttype
    value: Watergang met dijk
  - label: Bovenliggend object
    value: Watergangsectie
contentSections:
  - title: Overzicht / Samenhang
    summary: Watergang met dijk is een type watergangsectie waarin het waterprofiel en het dijkprofiel in dezelfde ruimtelijke sectie worden beschreven.
  - title: Afbakening
    summary: De afbakening omvat het deeltraject van de watergangsectie en de dijk of waterkerende zone voor zover die de profielopbouw en begrenzing bepaalt.
  - title: Onderdelen
    summary: De belangrijkste onderdelen zijn het waterprofiel, het dijkprofiel, bermen, eventuele taluds en de profielgrenzen.
    items:
      - title: Dijkprofiel
        text: Waterkerend profieldeel dat langs of naast de watergang ligt.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/dijkprofiel
      - title: Waterprofiel
        text: Doorsnede van de watergangsectie waaraan het dijkprofiel ruimtelijk is gekoppeld.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/waterprofiel
      - title: Berm
        text: Beheerbare strook of overgangszone tussen waterprofiel, dijkprofiel en omliggend terrein.
        href: /datastandaard/objectenhandboek/watersysteem/watergangsectie/berm
  - title: Geometrie
    summary: De geometrie wordt bepaald door de ligging van het waterprofiel, het dijkprofiel en de profielgrenzen binnen het deeltraject.
    items:
      - title: Overzicht
        text: Het overzicht toont hoe het waterprofiel en het dijkprofiel samen de ruimtelijke opbouw van de watergangsectie bepalen.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met dijk/Watergang met dijk overzicht.jpg
          alt: Overzicht van Watergang met dijk met waterprofiel en dijkprofiel.
        caption: Afbeelding 1. Overzicht van Watergang met dijk.
      - title: Dwarsprofiel
        text: Het dwarsprofiel laat zien hoe de dijk ruimtelijk samenhangt met de watergangsectie.
        image:
          src: /images/objectenhandboek/watersysteem/watergangsectie_watergang met dijk/Watergang met dijk profiellijn.png
          alt: Doorsnede van Watergang met dijk met profiellijn.
        caption: Afbeelding 2. Dwarsprofiel van Watergang met dijk.
productRelations:
  - title: Woordenboek
    text: Begrippen en definities voor watergangsecties en dijkprofielen.
    href: /datastandaard/woordenboek
  - title: Object Type Library
    text: Gestandaardiseerde eigenschappen en relaties van dit sectietype.
    href: /datastandaard/otl
  - title: Referentiedataset
    text: Voorbeelddata voor watergangsecties met dijk.
    href: /datastandaard/referentiedataset
  - title: Werkinstructies
    text: Werkafspraken voor het vastleggen van profiel en dijk.
    href: /datastandaard/werkinstructies
nextSteps:
  - title: Watergangsectie
    text: Terug naar het bovenliggende objecttype en de andere sectietypen.
    href: /datastandaard/objectenhandboek/watersysteem/watergangsectie
  - title: Watergangen
    text: Terug naar het hoofdobject van de waterganglijn.
    href: /datastandaard/objectenhandboek/watersysteem/watergangen
---

Deze detailpagina werkt `Watergang met dijk` uit als type naar compositie van `Watergangsectie`.
